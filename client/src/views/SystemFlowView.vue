    ...
    <div class="flex-1 ml-64 overflow-auto">
      ...
      <main class="p-8 max-w-6xl mx-auto">
        ...
        <div class="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl p-6 text-white">
          <h2 class="text-base font-bold mb-1">Ready to try it?</h2>
          <p class="text-indigo-200 text-sm mb-4">Submit a job and watch the entire flow happen in real time.</p>
          <div class="flex items-center gap-3">
            <button @click="$router.push('/submit-job')"
              class="flex items-center gap-2 px-5 py-2.5 bg-white text-indigo-700 text-sm font-bold rounded-lg hover:bg-indigo-50 transition-colors">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/>
              </svg>
              Submit a Job
            </button>
            <button @click="$router.push('/my-jobs')"
              class="flex items-center gap-2 px-5 py-2.5 bg-white/20 text-white text-sm font-medium rounded-lg hover:bg-white/30 transition-colors">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const taskStats = computed(() => {
  ...
  return [
    ...
    { label: 'Done',     value: t.done     ?? 0, dot: 'bg-green-500',  color: 'text-green-600'  },
